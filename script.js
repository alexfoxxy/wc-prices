...
(function ($) {
  $(document).ready(function () {
    ...
    /**
     * BEGIN Handle action dropdown changes
     */
    ...
    //EDN Handle action dropdown changes
    ...
    function editRegularPrice(action) {
      ...
      jQuery.each(selected, function (index, i) {
        ...
        var regular = accounting.unformat(
          products[i].regular_price,
          wooPrices.decimalSeparator
        );
        switch (action) {
          // New regular price
          case "newRegularPrice":
        ...
        products[i].regular_price = accounting.formatMoney(
          regular,
          "",
          2,
          "",
    ...
    function editSalePrice(action) {
      var value = $("#action-value").val(); // value
      var newFrom = $("#sale-from").val(); // Start Date
      var newTo = $("#sale-to").val(); // End Date
      var amount = getAmountValue(value); // Count
      var percent = getPercentValue(value);
      var selected = grid.getSelectedRows();
      var products = grid.getData();
      // Edit selected products
      jQuery.each(selected, function (index, i) {
        ...
        products[i].sale_price = accounting.formatMoney(
          sale,
          "",
          2,
          "",
    ...
    /**
     * Copy regular price in sale price
     */
    function getSalesPrices() {
      console.log();
    }
